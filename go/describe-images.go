...
// GOPATH=$PWD go build describe-images.go obdi_clientlib.go
...
func (t *Plugin) GetRequest(args *Args, response *[]byte) error {
	...
	if len(args.QueryString["executable_users"]) > 0 {
		for i := range args.QueryString["executable_users"] {
			params.ExecutableUsers = append(params.ExecutableUsers,
				&args.QueryString["executable_users"][i])
		}
	}

	// ImageIds List

	if len(args.QueryString["image_id"]) > 0 {
		for i := range args.QueryString["image_id"] {
			params.ImageIds = append(params.ImageIds,
				&args.QueryString["image_id"][i])
		}
	}

	// Owners List

	if len(args.QueryString["owners"]) > 0 {
		for i := range args.QueryString["owners"] {
			params.Owners = append(params.Owners,
				&args.QueryString["owners"][i])
		}
	}
	...
	if err != nil {
		t := "Error running DescribeImages: " + err.Error()
